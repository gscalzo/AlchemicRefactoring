/**
 * Created by giordanoscalzo on 13/10/2014.
 */
...
public class GildedRoseTest {
    ...
    private void backstagePassQuality(int sellIn, int startQuality, int expectedQuality) {
        ...
        whenWeUpdateTheQuality();
        assertEquals("quality for sellIn " + sellIn, expectedQuality, app.items.get(0).quality);
        assertEquals("new sellIn for old sellIn " + sellIn, sellIn - 1, app.items.get(0).sellIn);
    }
    ...
    private void thenItemBecomes(String name, int sellIn, int quality) {
        assertEquals(name, app.items.get(0).name);
        assertEquals("sellIn", sellIn, app.items.get(0).sellIn);
        assertEquals("quality", quality, app.items.get(0).quality);
    }
}